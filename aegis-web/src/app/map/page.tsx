...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    const prompt = [
      ...
      "Write exactly 4 bullet points describing what happened and why this event is occurring now.",
      "Each line must start with '- '.",
      "Prefix each bullet with either 'Confirmed:' or 'Inferred:'.",
      "Bullet structure requirement:",
      "- Bullet 1: factual event snapshot (what/where/when/actors/target).",
      "- Bullet 2: immediate catalyst or trigger behind this event.",
      "- Bullet 3: broader conflict driver/background (political, military, alliance, or economic pressure).",
      "- Bullet 4: likely near-term implication for this area (next days/weeks) grounded in evidence.",
      "If available, include weapon/interception details, named actors, and trigger/background from provided text.",
      "Use concrete event facts and numbers from provided content and full-article context when available.",
      "If details are sparse, infer the most likely causal context from related headlines and nearby same-event signals only (do not say 'unknown' or 'not reported').",
      ...
      "Do NOT mention confidence scores, magnitude scores, layer counts, model behavior, or why the model flagged/plotted the event.",
      "Avoid generic recap phrasing like 'multiple events occurred' without explaining mechanism/cause.",
