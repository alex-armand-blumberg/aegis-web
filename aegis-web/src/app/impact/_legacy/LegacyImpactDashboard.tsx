...
function filterAssetsBySelection(all: UserAsset[], selectedAssetIds?: string[]): UserAsset[] {
  if (!selectedAssetIds || selectedAssetIds.length === 0) return all;
  const selected = new Set(selectedAssetIds);
  return all.filter((asset) => selected.has(asset.id));
}

type LegacyImpactDashboardProps = {
  selectedAssetIds?: string[];
};

export function LegacyImpactDashboard({ selectedAssetIds }: LegacyImpactDashboardProps = {}) {
  ...
  const persistAssetsRef = useRef(false);

  useEffect(() => {
    persistAssetsRef.current = false;
    setAssets(filterAssetsBySelection(loadAssets(), selectedAssetIds));
    setFeedback(loadFeedback());
  }, [selectedAssetIds]);

  useEffect(() => {
    if (!persistAssetsRef.current) {
      persistAssetsRef.current = true;
      return;
    }
    saveAssets(assets);
  }, [assets]);
